...
int AecCoresize = sizeof(AecCorestruct);
  ...
  aecpc->far_pre_buf =
      WebRtc_CreateBuffer(PART_LEN2 + kResamplerBufferSize, sizeof(int32_t));
...
int32_t WebRtcAec_BufferFarend(void* aecInst,
  ...
  int16_t new_farend[MAX_RESAMP_LEN];
  const int32_t* farend_ptr = farend;
  ...
  while (WebRtc_available_read(aecpc->far_pre_buf) >= PART_LEN2) {
    ...
    {
      int32_t* ptmp = NULL;
      int32_t tmp[PART_LEN2];
      WebRtc_ReadBuffer(aecpc->far_pre_buf, (void**)(&ptmp),
                        tmp, PART_LEN2);
...
int32_t WebRtcAec_Process(void* aecInst,
  ...
  //aecpc->data_dumper->DumlpRaw("aec_system_delay", 1, &far_buf_size_samples);
...
static void EstBufDelayNormal(Aec* aecInst) {
  ...
  aecInst->filtDelay = WEBRTC_SPL_MAX(
      0, (int16_t)(0.8 * aecInst->filtDelay + 0.2 * current_delay));
...
static void EstBufDelayExtended(Aec* aecInst) {
  ...
  if (aecInst->filtDelay == -1) {
    ...
    aecInst->filtDelay = WEBRTC_SPL_MAX(
        0,
        (int16_t)(0.95 * aecInst->filtDelay + 0.05 * current_delay));
  }
