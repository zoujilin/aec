...
void wavWrite_int16(char *filename, int16_t *buffer, size_t sampleRate, size_t totalSampleCount, uint32_t channels) {
    drwav_data_format format = {};
    format.container = drwav_container_riff;     // <-- drwav_container_riff = normal WAV files, drwav_container_w64 = Sony Wave64.
    format.format = DR_WAVE_FORMAT_PCM;          // <-- Any of the DR_WAVE_FORMAT_* codes.
...
int16_t *wavRead_int16(char *filename, uint32_t *sampleRate, uint64_t *totalSampleCount, uint32_t *channels) {
    int16_t *buffer = drwav_open_and_read_file_s16(filename, (unsigned int*)channels, (unsigned int*)sampleRate, totalSampleCount);
    if (buffer == NULL) {
        printf("Failed in wav file read.");
    }
    return buffer;
}
